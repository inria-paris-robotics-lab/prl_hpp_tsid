...
from prl_hpp.ur5 import ur5_robot, ur5_planner, ur5_commander_left_arm as commander_L, ur5_commander_right_arm as commander_R
pi = 3.1415926

ur5_planner.lock_grippers()
ur5_planner.lock_right_arm()
ur5_planner.set_velocity_limit(0.25)
ur5_planner.set_acceleration_limit(0.25)
...
q_curr = ur5_robot.get_meas_q()
ur5_planner.v(q_curr)
...
paths = ur5_planner.make_pick_and_place(ur5_robot.left_gripper_name, pose_1, pose_2, approach_distance = 0.2)

input("Solution found. Press enter to display path")
ur5_planner.pp(paths[0].id)
input("Press enter to display path")
ur5_planner.pp(paths[1].id)
input("Press enter to display path")
ur5_planner.pp(paths[2].id)
